...
\textbf{Amortized analysis} provides a method for analysing the average cost of operations over a sequence, even when individual operations exhibit high variance in their execution time~\cite{online:mit6046:amortized:2015}. Unlike average-case analysis, which relies on probabilistic assumptions about input distributions, amortized analysis guarantees worst-case bounds on the total cost of any sequence of operations.

The \textbf{aggregate method} computes amortized cost by dividing the total worst-case cost $T(n)$ of $n$ operations by $n$. If $T(n) \in O(f(n))$, then each operation has amortized cost $O(f(n)/n)$.
...
\end{equation}
Dividing by $n$ yields an amortized cost of $O(1)$ per insertion, despite occasional $O(n)$ operations.
